...
python3 - "$INFO_PLIST_TEMPLATE" "$staging/Contents/Info.plist" \
  "$APP_NAME" "$APP_EXECUTABLE" "$BUNDLE_ID" "$APP_VERSION" "$BUILD_NUMBER" "$MINIMUM_MACOS_VERSION" \
  "$SPARKLE_FEED_URL" "$SPARKLE_PUBLIC_ED_KEY" <<'PY'
import plistlib
import sys

(source, destination, app_name, executable, bundle_id, version, build, minimum,
 feed_url, public_ed_key) = sys.argv[1:]
...
plist["SUFeedURL"] = feed_url
plist["SUPublicEDKey"] = public_ed_key
with open(destination, "wb") as handle:
    plistlib.dump(plist, handle, fmt=plistlib.FMT_XML, sort_keys=False)
PY
printf 'APPL????' > "$staging/Contents/PkgInfo"

embed_sparkle_framework "$staging"

validate_macho "$staging/Contents/MacOS/$APP_EXECUTABLE" arm64 x86_64
validate_macho "$staging/Contents/Frameworks/Sparkle.framework/Versions/B/Sparkle" arm64 x86_64
