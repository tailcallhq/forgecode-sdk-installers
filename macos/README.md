...
The release pipeline validates the universal application slices, minimum OS, dependencies, and exact runtime-free payload inventory; replaces the development seal by signing the application executable and bundle with hardened runtime; signs the DMG; and submits the DMG for notarization **without waiting**. Only the DMG is submitted — notarizing the DMG notarizes the app inside it recursively, so the app is never submitted on its own. The submission id is recorded next to the DMG, and stapling happens later, in a decoupled step (`scripts/staple-dmg.sh`, driven by the scheduled finalize workflow), so the build never blocks on Apple's notary queue. The pipeline compares the complete mounted app bundle, validates the `/Applications` symlink, and atomically writes and verifies the exact manifest/checksum inventory plus retained notarization logs. Real signing credentials are not needed for development: `scripts/package-unsigned.sh` runs the ad-hoc validation path.

Useful individual commands are `scripts/assemble-app.sh`, `scripts/sign-app.sh`, `scripts/create-dmg.sh`, `scripts/sign-dmg.sh`, `scripts/submit-dmg.sh` (non-blocking notary submission), `scripts/staple-dmg.sh` (poll + staple once accepted), and `scripts/verify-release.sh`. `APP_VERSION`, `BUILD_NUMBER`, and `MINIMUM_MACOS_VERSION` accept numeric dot-separated plist versions only.
